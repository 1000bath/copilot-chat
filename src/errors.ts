...
export class DekRequestError extends Error {
  constructor(
    message: string,
    readonly userMessage: string = message,
  ) {
    super(message);
    this.name = "DekRequestError";
  }
}

export function buildDekRequestError(
  ...
  return new DekRequestError(requestMessage, userMessage);
}
...
function describeRouterUnavailable(apiError: ParsedApiError, fallback: string): string {
  ...
  return "Dek's router has no healthy backend for this model right now. Retry in a few seconds, or switch to another model.";
}
...
function classifyRateLimit(apiError: ParsedApiError, status: number): string {
  ...
  if (compactCode.includes("gosubscriptionrollinglimitexceeded")) {
    return "5-hour Dek Go usage limit reached";
  }
  if (compactCode.includes("gosubscriptionweeklylimitexceeded")) {
    return "weekly Dek Go usage limit reached";
  }
  if (compactCode.includes("gosubscriptionmonthlylimitexceeded")) {
    return "monthly Dek Go usage limit reached";
  }
  if (code.includes("subscriptionquota") || code.includes("quota")) {
    return "Dek quota exceeded";
  }
  return status === 429 ? "rate limit exceeded" : "Dek usage limit reached";
}
