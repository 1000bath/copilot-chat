...
export function buildFamilyThinkingSchema(
  ...
  if (family === "minimax") {
    // Dek transform.ts only defines none/thinking for minimax-m3, and
...
/**
 * Maps the per-family Thinking settings to the request fields each Dek
...
export function buildThinkingPayload(modelId: string, thinking: ThinkingSettings, hasImageInput = false): Record<string, unknown> {
  ...
  // The Dek gateway forwards reasoning.effort to the OpenAI Responses API.
  ...
  if (/^mimo-/i.test(modelId)) {
    ...
    // Supported efforts: low, medium, high (per Dek upstream defaults).
    //
    ...
    // If the Dek gateway rejects budget_tokens (HTTP 400 "extra inputs"),
  ...
  if (/^minimax-/i.test(modelId)) {
    // Dek transform.ts maps minimax-m3 to thinking: { type: "disabled"|"adaptive" }
