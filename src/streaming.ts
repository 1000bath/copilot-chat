...
export async function streamChatCompletions(options: StreamRequestOptions): Promise<void> {
  ...
  if (extractor.emittedText === 0 && extractor.emittedTools === 0) {
    options.output?.appendLine(
      `[warn] empty response from model=${options.modelId} (no text, no tool calls, no reasoning). Try a different free model or enable dekgo.debugReasoning to inspect raw SSE.`,
    );
...
interface StreamDekResponseOptions extends StreamRequestOptions {
...
async function streamDekResponse(options: StreamDekResponseOptions): Promise<void> {
  ...
  try {
    ...
    // ------------------------------------------------------------------
    // NOTE: We do NOT gzip-compress the payload.  The Dek proxy
    ...
    if (!response.ok) {
      ...
      const capacityHint =
        options.capacityLimitedModelNotes?.[options.modelId] && response.status >= 500
          ...
          : "";
      const requestError = buildDekRequestError(
    ...
    if (abortReason === "request-timeout") {
      const requestError = new DekRequestError(
...
class OpenAiResponseExtractor {
  ...
  extractStreamParts(data: unknown): vscode.LanguageModelResponsePart[] {
    ...
    // Gateways that omit `finish_reason` entirely (e.g. Dek Go for
  ...
  private flushToolCalls(): vscode.LanguageModelToolCallPart[] {
    ...
    const parts = calls.map(
      (call, index) => new vscode.LanguageModelToolCallPart(call.id || `dekgo-tool-${Date.now()}-${index}`, call.name, call.input),
    );
   ...
   * Dek Go gateway for gpt-5.6-luna, issue #93), so a final flush here
    ...
    const parts = toolCalls.map(
      (toolCall, index) =>
        new vscode.LanguageModelToolCallPart(
          toolCall.id || `dekgo-tool-${Date.now()}-${index}`,
          toolCall.name,
...
function extractAnthropicParts(data: unknown): vscode.LanguageModelResponsePart[] {
  ...
  for (const block of data.content) {
    ...
    if (block.type === "tool_use" && typeof block.name === "string") {
      const id = typeof block.id === "string" ? block.id : `dekgo-tool-${Date.now()}`;
...
function toolCallPartsFromOpenAiMessage(toolCalls: unknown): vscode.LanguageModelToolCallPart[] {
  ...
  return toolCalls
    ...
    .map((toolCall, index) => {
      ...
      const id = typeof toolCall.id === "string" ? toolCall.id : `dekgo-tool-${Date.now()}-${index}`;
