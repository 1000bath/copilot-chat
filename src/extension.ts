...
import { DekRequestError } from "./errors";
import {
...
import { buildDekGatewayAuthHeaders } from "./openCodeAuth";
import {
...
const PROVIDERS: Record<ProviderDefinition["vendor"], ProviderDefinition> = (() => {
  ...
  const zen: ProviderDefinition = {
    ...
    filterModel: (modelId) =>
      vscode.workspace.getConfiguration("dekgo").get("freeOnly", true)
        ? modelId.endsWith("-free") || FREE_ZEN_MODEL_IDS.has(modelId)
        : true,
  };
  const dekgateway: ProviderDefinition = {
    vendor: DEKGATEWAY_VENDOR,
    displayName: "DekGateway",
    modelNamePrefix: "DekGateway",
    modelsUrl: "http://localhost:8787/v1/models",
    chatCompletionsUrl: "http://localhost:8787/v1/chat/completions",
    messagesUrl: "http://localhost:8787/v1/chat/completions",
    testModelId: "local/ai-free",
    fallbackModels: ["local/ai-free"],
  };

  return {
    [GO_VENDOR]: go,
    [ZEN_VENDOR]: zen,
    [DEKGATEWAY_VENDOR]: dekgateway,
    [AGENT_GO_VENDOR]: { ...providerVariant(go, AGENT_GO_VENDOR, "Dek Go (Agents)"), isAgentVariant: true, baseVendor: GO_VENDOR },
    [AGENT_ZEN_VENDOR]: {
      ...providerVariant(zen, AGENT_ZEN_VENDOR, "Dek Zen (Agents)"),
      isAgentVariant: true,
...
interface DekModel extends vscode.LanguageModelChatInformation {
 ...
 * Dek provider transform (anomalyco/opencode, packages/opencode/src/provider/transform.ts):
 *
 ...
 * failed" rejections from Dek Go. Larger images are replaced with a
 ...
 *     and forwards a multi-MB request that the Dek Go gateway rejects.
...
// Models live on the Dek Zen gateway but with constrained GPU capacity.
// They were re-enabled by the Dek team after a brief shutdown
...
export function activate(context: vscode.ExtensionContext) {
  const goUsageLogChannel = vscode.window.createOutputChannel("Dek Go Usage");
  ...
  const goProvider = new DekProvider(context, PROVIDERS[GO_VENDOR]);
  const zenProvider = new DekProvider(context, PROVIDERS[ZEN_VENDOR]);
  const dekgatewayProvider = new DekProvider(context, PROVIDERS[DEKGATEWAY_VENDOR]);
  const modelInfoProviders: DekProvider[] = [goProvider, zenProvider, dekgatewayProvider];
  ...
  const subscriptions: vscode.Disposable[] = [
    ...
    vscode.lm.registerLanguageModelChatProvider(DEKGATEWAY_VENDOR, dekgatewayProvider),
    // Compatibility registrations preserve existing provider selections from pre-Dek releases.
    vscode.lm.registerLanguageModelChatProvider("opencodego", goProvider),
    vscode.lm.registerLanguageModelChatProvider("opencodezen", zenProvider),
    vscode.lm.registerLanguageModelChatProvider("jonusgateway", dekgatewayProvider),
    // Legacy command IDs remain callable by old keybindings and command URIs.
    ...[
      ["opencodego.manage", "dekgo.manage"],
      ["opencodego.setApiKey", "dekgo.setApiKey"],
      ["opencodego.refreshModels", "dekgo.refreshModels"],
      ["opencodego.configureUtilityModels", "dekgo.configureUtilityModels"],
      ["opencodego.diagnostics", "dekgo.diagnostics"],
      ["opencodego.modelPickerDiagnostics", "dekgo.modelPickerDiagnostics"],
      ["opencodego.setThinkingEffort", "dekgo.setThinkingEffort"],
      ["opencodego.showUsageDetails", "dekgo.showUsageDetails"],
      ["opencodego.setUsageTargets", "dekgo.setUsageTargets"],
      ["opencodego.showUsageQuickPick", "dekgo.showUsageQuickPick"],
      ["opencodego.renameActiveProfile", "dekgo.renameActiveProfile"],
      ["opencodego.deleteProfile", "dekgo.deleteProfile"],
      ["opencodego.configureVisionProxy", "dekgo.configureVisionProxy"],
      ["opencodezen.diagnostics", "dekzen.diagnostics"],
      ["opencodezen.manage", "dekzen.manage"],
      ["opencodezen.refreshModels", "dekzen.refreshModels"],
      ["jonusgateway.manage", "dekgateway.manage"],
      ["jonusgateway.setApiKey", "dekgateway.setApiKey"],
      ["jonusgateway.refreshModels", "dekgateway.refreshModels"],
      ["jonusgateway.diagnostics", "dekgateway.diagnostics"],
    ].map(([legacyId, canonicalId]) =>
      vscode.commands.registerCommand(legacyId, (...args: unknown[]) => vscode.commands.executeCommand(canonicalId, ...args)),
    ),
    vscode.commands.registerCommand("dekgo.manage", () => goProvider.manage()),
    vscode.commands.registerCommand("dekgo.diagnostics", () => goProvider.showDiagnostics()),
    vscode.commands.registerCommand("dekgo.setApiKey", () => goProvider.setApiKey()),
    vscode.commands.registerCommand("dekgo.refreshModels", () => goProvider.refreshModels()),
    vscode.commands.registerCommand("dekgo.configureUtilityModels", () => configureUtilityModels()),
    vscode.commands.registerCommand("dekzen.diagnostics", () => zenProvider.showDiagnostics()),
    vscode.commands.registerCommand("dekzen.manage", () => zenProvider.manage()),
    vscode.commands.registerCommand("dekzen.refreshModels", () => zenProvider.refreshModels()),
    vscode.commands.registerCommand("dekgateway.manage", () => dekgatewayProvider.manage()),
    vscode.commands.registerCommand("dekgateway.diagnostics", () => dekgatewayProvider.showDiagnostics()),
    vscode.commands.registerCommand("dekgateway.refreshModels", () => dekgatewayProvider.refreshModels()),
    vscode.commands.registerCommand("dekgateway.setApiKey", () => dekgatewayProvider.setApiKey()),
    vscode.commands.registerCommand("dekgo.modelPickerDiagnostics", () => showModelPickerDiagnostics()),
    vscode.commands.registerCommand("dekgo.setThinkingEffort", () => showThinkingEffortPicker()),
    vscode.commands.registerCommand("dekgo.showUsageDetails", () => showUsageWebview(context)),
    vscode.commands.registerCommand("dekgo.setUsageTargets", async () => {
      ...
      if (targets) {
        ...
        refreshGoUsageStatusBar();
        vscode.window.showInformationMessage("Dek Go usage targets updated.");
      }
    }),
    vscode.commands.registerCommand("dekgo.showUsageQuickPick", async () => {
      const tracker = activeGoUsageTracker();
      if (!tracker) return;
    ...
    vscode.commands.registerCommand("dekgo.deleteProfile", async () => {
      ...
      ctx.globalState.update(`dekgo.usageLog.v1.${fp}`, []);
      ctx.globalState.update(`dekgo.usageBaseline.v1.${fp}`, {});
      ctx.globalState.update(`dekgo.sessionCosts.v1.${fp}`, []);
    ...
    vscode.commands.registerCommand("dekgo.configureVisionProxy", async () => {
  ...
  const enableAgents = vscode.workspace.getConfiguration("dekgo").get<boolean>("agentsWindow", true);
  if (enableAgents) {
    const agentGoProvider = new DekProvider(context, PROVIDERS[AGENT_GO_VENDOR]);
    const agentZenProvider = new DekProvider(context, PROVIDERS[AGENT_ZEN_VENDOR]);
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("dekgo.showUsageStatusBar")) {
        resetUsageStatusBar();
      }
      if (event.affectsConfiguration("dekgo.showProviderPrefix")) {
...
async function warmModelPickerMetadata(): Promise<void> {
  ...
  if (vscode.workspace.getConfiguration("dekgo").get<boolean>("agentsWindow", true)) {
...
async function showModelPickerDiagnostics(): Promise<void> {
  ...
  const doc = await vscode.workspace.openTextDocument({
    content: ["# Dek Model Picker Diagnostics", "", ...sections].join("\n"),
    language: "markdown",
  });
...
async function showThinkingEffortPicker(): Promise<void> {
  ...
  if (!choice) return;
  const cfg = vscode.workspace.getConfiguration("dekgo.thinking");
  await cfg.update(family.family.key, choice, vscode.ConfigurationTarget.Global);
  vscode.window.showInformationMessage(`Dek Thinking — ${family.family.label}: ${choice}`);
}
...
function shouldShowUsageStatusBar(): boolean {
  return vscode.workspace.getConfiguration("dekgo").get("showUsageStatusBar", true);
}
...
function resetUsageStatusBar(): void {
  ...
  usageStatusBarItem.text = "Dek";
  usageStatusBarItem.tooltip = "Dek usage summary";
...
function ensureGoUsageStatusBar(context: vscode.ExtensionContext): void {
  ...
  goUsageStatusBarItem.command = "dekgo.showUsageQuickPick";
...
function refreshGoUsageStatusBar(): void {
  ...
  const tracker = activeGoUsageTracker();
  if (!tracker) {
    goUsageStatusBarItem.text = "Dek Go";
...
function showUsageWebview(context: vscode.ExtensionContext): void {
  ...
  usageWebviewPanel = vscode.window.createWebviewPanel("dekgo.usageWebview", "Dek Usage Summary", vscode.ViewColumn.Beside, {
    enableScripts: false,
...
function updateWebviewContent(): void {
  ...
  const activeProfile = findProfile(profilesCache, activeProfileFingerprint);
  const profileLabel = activeProfile?.label ?? "Dek Go";
  ...
  usageWebviewPanel.webview.html = `
    ...
    <head>
      ...
      <title>Dek Usage Summary — ${escapeSvg(profileLabel)}</title>
      <style>
        body {
          display: flex;
...
function buildUsageTooltip(
  ...
  const commands = ["dekgo.setUsageTargets"];
  if (nonLegacyCount(profilesCache) > 0) {
    commands.push("dekgo.renameActiveProfile");
  }
  ...
  md.appendMarkdown("\n\n[$(pencil) Set spent targets](command:dekgo.setUsageTargets)");
  if (nonLegacyCount(profilesCache) > 0) {
    md.appendMarkdown(" \u00B7 [$(pencil) Rename](command:dekgo.renameActiveProfile)");
  }
  return md;
}
...
async function showUsageTargetEditor(tracker: GoUsageTracker): Promise<UsageBaselineTargets | undefined> {
  ...
  const monthlyDayStr = await vscode.window.showInputBox({
    title: "Dek Go — Monthly Reset Day",
  ...
  const monthlyHourStr = await vscode.window.showInputBox({
    title: "Dek Go — Monthly Reset Hour",
...
function buildUsageTooltipSvg(
  ...
  const svgTitle = escapeSvg(profileLabel ? `${profileLabel} - Usage` : "Dek Go - Usage");
...
class DekProvider implements vscode.LanguageModelChatProvider<DekModel> {
   ...
   * `dekgo` group does not affect `dekzen`.
   */
  ...
  private storeReasoningContent(toolCallIds: string[], reasoningContent: string): void {
    ...
    if (this.reasoningContentByToolCallId.size > DekProvider.REASONING_CACHE_LIMIT) {
      const excess = this.reasoningContentByToolCallId.size - DekProvider.REASONING_CACHE_LIMIT;
  ...
  private getOutputChannel(): vscode.OutputChannel {
    if (!this.outputChannel) {
      this.outputChannel = vscode.window.createOutputChannel("Dek");
  ...
  private async getMetadataSnapshot(): Promise<CachedModelMetadataSnapshot> {
    return getDekModelMetadata(this.context, this.getOutputChannel());
  }
  ...
  private async refreshMetadataAndModels(): Promise<void> {
    await clearDekModelMetadataCache(this.context);
  ...
  /**
   * Public entry point for the `Dek <Vendor>: Refresh Models` commands.
   *
   * CONTRACT:
   ...
   * was only reachable as a sub-item inside `Dek Go: Manage Provider`
  ...
  async provideLanguageModelChatInformation(
    ...
    //
    //    See issue #86: non-agent `dekzen` returned 0 models when the key
    ...
    const metadataSnapshot = await this.getMetadataSnapshot();
    const showProviderPrefix = vscode.workspace.getConfiguration("dekgo").get<boolean>("showProviderPrefix", true);
    ...
    const results = models.flatMap((modelId) => {
      ...
      const sharedFields: Omit<DekModel, "id" | "targetChatSessionType"> = {
        rawModelId: modelId,
      ...
      const info: DekModel = { ...sharedFields, id: fpEffectiveModelId };
  ...
  async provideLanguageModelChatResponse(
    ...
    // `400 Upstream request failed` rejections from Dek Go (issue #38
    ...
    const requestHeaders = buildDekRequestHeaders(messages, options, rawModelId);
    ...
    try {
      ...
      if (routing.endpointKind === "messages") {
        await runStreamAnthropicMessages({
          ...
          authHeaders: buildDekGatewayAuthHeaders("messages", apiKey),
      ...
      if (routing.endpointKind === "responses") {
        await runStreamResponsesApi({
          ...
          authHeaders: buildDekGatewayAuthHeaders("responses", apiKey),
          requestHeaders,
          progress,
          token,
          output: outputChannel,
      ...
      if (routing.endpointKind === "google") {
        await runStreamGoogleGenerateContent({
          ...
          streamIdleTimeoutMs: settings.streamIdleTimeoutMs,
          contextWindowOutputBuffer,
          authHeaders: buildDekGatewayAuthHeaders("google", apiKey),
          capacityLimitedModelNotes: CAPACITY_LIMITED_MODEL_NOTES,
          onTransportSummary,
          stripThinkTags: settings.stripThinkTags,
          onReasoningContent: (toolCallIds, reasoningContent) => {
      ...
      await runStreamChatCompletions({
        ...
        authHeaders: buildDekGatewayAuthHeaders("chat-completions", apiKey),
        requestHeaders,
        progress,
        token,
        output: outputChannel,
      ...
      if (error instanceof DekRequestError) {
  ...
  /**
   * Fetch the live model list from the Dek gateway.
   *
   * CONTRACT:
...
async function clearDekModelMetadataCache(context: vscode.ExtensionContext): Promise<void> {
...
async function getDekModelMetadata(
  ...
  if (cached) {
    ...
    void refreshDekModelMetadata(context, output);
    return cached;
  }

  return refreshDekModelMetadata(context, output);
}

async function refreshDekModelMetadata(
  context: vscode.ExtensionContext,
  output?: vscode.OutputChannel,
): Promise<CachedModelMetadataSnapshot> {
...
// The official Dek client sends these headers on every request. The Zen
...
function buildDekRequestHeaders(
  messages: readonly vscode.LanguageModelChatRequestMessage[],
  options: vscode.ProvideLanguageModelChatResponseOptions,
  modelId: string,
  ...
  for (const part of message.content) {
    ...
    if (part instanceof vscode.LanguageModelToolResultPart) {
      ...
      if (toolImageParts.length > 0) {
        ...
        // messages to have a plain string content. The Dek Go gateway
...
// per-request selection back to the right Dek request field.
...
function getSettings(): ApiSettings {
  const config = vscode.workspace.getConfiguration("dekgo");
...
async function proxyVision(
  ...
  // 2. Vendor:id partial (e.g. "dekgo:mimo-v2.5")
  ...
  if (!visionModels || visionModels.length === 0) {
    throw new Error(`Vision model "${visionModelId}" not found. ` + `Run "Dek Go: Configure Vision Proxy" to see available models.`);
  }
...
const VISION_PROXY_MODEL_ID_KEY = "dekgo.visionProxyModelId";
const VISION_PROXY_PROMPT_KEY = "dekgo.visionProxyPrompt";
 ...
 * stored in globalState via the "Dek Go: Configure Vision Proxy" command).
 */
...
async function showVisionProxyPicker(context: vscode.ExtensionContext): Promise<void> {
  ...
  const modelItems = allModels
    .map((m) => {
      ...
      const fromName = [...visionCapableIds].some((id) => m.id.includes(id.replace(/^(dekgo|dekzen|copilot):/, "")));
