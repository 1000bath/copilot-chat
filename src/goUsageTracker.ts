...
const STORAGE_KEY = "dekgo.usageLog.v1";
const BASELINE_STORAGE_KEY = "dekgo.usageBaseline.v1";
const SESSION_COSTS_KEY = "dekgo.sessionCosts.v1";
const MAX_LOG_ENTRIES = 2000;

/** Dek Go subscription limits in USD, from https://opencode.ai/docs/go */
...
export interface UsageSummary {
  ...
  hasData: boolean;
  /** When true, cost data comes from the Dek CLI SQLite database
...
// ─── Dek SQLite history reader (same source as OpenUsage) ───────────────
...
function readDekHistory(): HistoryRow[] | null {
...
export class GoUsageTracker {
  ...
  setManualSpentTargets(targets: UsageBaselineTargets): void {
    ...
    const isPerProfile = this.storageKeySuffix.length > 0;
    const sqliteRows = isPerProfile ? null : readDekHistory();
...
export function formatGoUsageStatusBarText(summary: UsageSummary): string {
  if (!summary.hasData) return "Dek Go";
...
export function formatGoUsageTooltip(summary: UsageSummary): vscode.MarkdownString {
  ...
  md.appendMarkdown("**Dek Go — Usage Limits**\n\n");
...
export function buildUsageQuickPickItems(summary: UsageSummary): vscode.QuickPickItem[] {
  ...
  if (isEmpty) {
    items.push({
      ...
      detail: "Send a chat message to any Dek Go model to start tracking usage.",
      alwaysShow: true,
    });
  }
  ...
  items.push({
    label: "$(link-external) Open Dek console",
