  ...
  describe("GoUsageTracker", () => {
    ...
    describe("record()", () => {
      ...
      it("skips records when providerDisplayName does not contain 'go'", () => {
        const tracker = new GoUsageTracker(createMockContext());
        tracker.record(makeSummary({ providerDisplayName: "Dek Zen", sessionId: "s1" }));
      ...
      it("persists data to globalState after record()", () => {
        ...
        const storedEntries = context.globalState.get("dekgo.usageLog.v1", []);
        assert.equal(storedEntries.length, 1);

        const storedSessions = context.globalState.get<SessionSummary[]>("dekgo.sessionCosts.v1", []);
