 ...
 * - buildDekGatewayAuthHeaders() from openCodeAuth.ts
 *
 * For EACH model, tests ALL thinking/reasoning parameter combinations against
 * the live Dek API to verify what actually works.
 *
 * Usage:
...
import { buildDekGatewayAuthHeaders } from "../src/openCodeAuth.js";
...
async function testParameter(model: ModelInfo, test: ParamTest): Promise<TestResult> {
  ...
  const authHeaders = buildDekGatewayAuthHeaders(routing.endpointKind, API_KEY!);
