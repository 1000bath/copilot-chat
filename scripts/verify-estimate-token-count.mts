...
function generateChatPayload(targetTokens: number, hasToolCalls: boolean): string {
  ...
  messages.push(
    JSON.stringify({
      role: "system",
      content:
        "You are Dek Go BYOK Provider, an expert programming assistant integrated with VS Code Copilot Chat. You help users with coding questions, debugging, code review, architecture design, and general software engineering tasks across multiple programming languages and frameworks. You have access to workspace files and tools.",
    }),
  );
