...
Enable GitHub Copilot Chat users to attach images (screenshots, photos, diagrams) when using Dek Go/Zen models that support multimodal input. Prior to this change, all models reported `imageInput: false` regardless of their actual vision capability, and the extension did not handle `LanguageModelDataPart` image content from Copilot.

---

## Problem

1. **`modelCapabilities()` was hardcoded.** The function returned `imageInput: false` and `supportsImageToText: false` for every model, preventing Copilot from showing the image attachment button and from sending image data to any Dek model.
...
Added `VISION_CAPABLE_MODELS` — a `Set<string>` containing model IDs that support multimodal image input through the Dek gateway:
