...
Adds manual Go usage target configuration and switches cost estimation to use the live `models.dev` pricing snapshot (with the bundled static table kept only as a last-resort fallback). Closes [#23](https://github.com/1000bath/dek-copilot-chat/issues/23). Landed via PR [#50](https://github.com/1000bath/dek-copilot-chat/pull/50) by [@Wallacy](https://github.com/Wallacy).
...
1. **No manual override.** Users who topped up mid-cycle, or whose real Dek Go dashboard showed a different spent amount than the client-side estimate, had no way to reconcile the tracker with reality. The displayed percentages drifted from the actual subscription state.
...
| File                    | Change                                                                                                                                                                                                                                                                                                   |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `package.json`          | New command contribution: `dekgo.setUsageTargets` → "Dek Go: Set Usage Targets…"                                                                                                                                                                                                               |
...
### New Command: `dekgo.setUsageTargets`
...
3. **`md.isTrusted = true` without `supportedCommands`.** The tooltip command link may render as plain text in some VS Code versions. Adding `md.supportedCommands = ["dekgo.setUsageTargets"]` makes the intent explicit. Worth a quick hover test to confirm the link is clickable.
...
- Issue: [#23](https://github.com/1000bath/dek-copilot-chat/issues/23)
- PR: [#50](https://github.com/1000bath/dek-copilot-chat/pull/50) by [@Wallacy](https://github.com/Wallacy)
