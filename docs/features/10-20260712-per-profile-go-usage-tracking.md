...
**PR:** [#75](https://github.com/1000bath/dek-copilot-chat/pull/75)  
**Issue:** [#63](https://github.com/1000bath/dek-copilot-chat/issues/63)  
**Merged:** 2026-07-12  
...
Users with multiple Dek Go subscriptions (e.g. work + personal) could not use separate Go accounts in the same VS Code window. The usage panel mixed data from all keys because:
...
- `dekgo.profiles.v1` — profile registry array
- `dekgo.activeProfile.v1` — active profile fingerprint
- `dekgo.migratedTo.v1` — one-time migration flag
...
When `storageKeySuffix` is set (multi-profile mode), the tracker skips `readDekHistory()` entirely. This is necessary because `opencode.db` has no API key column, so reading it would mix quota from all accounts. Instead, the tracker uses only extension-tracked entries (which are namespaced per profile).
...
- `dekgo.renameActiveProfile` — rename via input box
- `dekgo.deleteProfile` — select from list, confirm, then delete with cleanup
