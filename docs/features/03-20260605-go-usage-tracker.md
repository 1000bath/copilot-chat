...
1. **No REST API** — Dek does not expose `/usage`, `/billing`, `/subscription`, `/quota`, or any public endpoint for usage data. All billing functions (`validateBilling()`, `queryLiteSubscription`) are server-side only.
...
3. **Dek Go Pricing** — Full per-model pricing available at `opencode.ai/docs/go`:
...
- Command registered: `dekgo.showUsage`
...
- Usage log stored in VS Code `globalState` under key `dekgo.usageLog.v1`
...
"/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code" \
  --install-extension dek-copilot-chat-0.2.0.vsix --force
```
...
**Status:** ✅ Solved | **Issue:** [#59](https://github.com/1000bath/dek-copilot-chat/issues/59) | **Contributor:** [@Wallacy](https://github.com/Wallacy)
...
```text
opencode.db (SQLite)
  → readDekHistory() — sqlite3 CLI, 5s timeout
