...
Implemented a persistent Webview panel for Dek Go usage details that stays open in the editor area, similar to how GitHub Copilot Chat shows quota information. Previously, usage data was only visible via a transient hover tooltip on the status bar icon — the tooltip disappeared as soon as the mouse moved away.
...
| P1  | Register `dekgo.showUsageDetails` command | `package.json`, `src/extension.ts` | New command entry point for click action                                     |
| P2  | Assign command to status bar item              | `src/extension.ts`                 | `goUsageStatusBarItem.command = "dekgo.showUsageDetails"` enables click |
...
| P7  | Activation event + command contribution        | `package.json`                     | `onCommand:dekgo.showUsageDetails` + command metadata                   |
...
- `package.json` — New command `dekgo.showUsageDetails`, new activation event
