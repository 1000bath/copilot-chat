...
# Agents Window Model Visibility — Dek Models in Copilot CLI Picker

**Topic:** models / vscode / agents-window
**Updated:** 2026-06-15
**Tags:** #models #vscode #agents-window #targetChatSessionType #marketplace #copilotcli
**GitHub Issue:** [1000bath/dek-copilot-chat#11](https://github.com/1000bath/dek-copilot-chat/issues/11)
**GitHub Issue:** [1000bath/dek-copilot-chat#41](https://github.com/1000bath/dek-copilot-chat/issues/41) (duplication regression)
**GitHub PR:** [#39](https://github.com/1000bath/dek-copilot-chat/pull/39) (by [@Marinski](https://github.com/Marinski)) — initial implementation
**GitHub PR:** [#42](https://github.com/1000bath/dek-copilot-chat/pull/42) (by [@Marinski](https://github.com/Marinski)) — opt-in gate fix
...
Dek Go and Dek Zen models now appear in the VS Code **Agents window** model picker when starting a Copilot CLI / Background agent session. Previously they were only visible in the regular Chat view.
...
| ------------- | ------------------------------------------ | ----------------------- | ----------------------------------- |
| General       | `dekgo:deepseek-v4-flash`             | _(none)_                | Chat view                           |
| Agents-window | `dekgo:deepseek-v4-flash::agent-host` | `"copilotcli"`          | Agents window > Copilot CLI session |

**Fix for #41 (PR #42):** Gated the `::agent-host` duplicate behind `dekgo.showInAgentsWindow` (default `false`). When enabled, the duplicate gets an `(Agents)` name suffix.
...
Register agent models under **dedicated vendor IDs** (`dekgo-agent`, `dekzen-agent`) so each vendor group shows only its variant in the Manage panel.
...
- **Manage panel is clean** — agent vendors are hidden by default via `when` clause (`dekgo.showAgentModelsInManagePanel`, default `false`)
...
| ----------------------------------------- | ------- | ----------------------------------- |
| `dekgo.agentsWindow`                 | `true`  | Register agent providers at runtime |
| `dekgo.showAgentModelsInManagePanel` | `false` | Show agent vendors in Manage panel  |
...
"extensions.supportAgentsWindow": {
    "1000bath.dek-copilot-chat": true
}
```

Without this setting, the extension will not load in the Agents window process and no Dek models will appear in the picker, regardless of the `targetChatSessionType` registration.
...
return models.flatMap((modelId) => {
  ...
  const info: DekModel = { ...sharedFields, id: effectiveModelId };
...
_Implemented by @Marinski in PR [#39](https://github.com/1000bath/dek-copilot-chat/pull/39). Resolves Issue [#11](https://github.com/1000bath/dek-copilot-chat/issues/11)._
