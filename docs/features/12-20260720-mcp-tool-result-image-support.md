...
| ----------------------- | ------------------------------------------------------------- | ------------------------------------------------------ |
| OpenAI chat-completions | ✅ Native — array content forwarded as-is                     | Used by Kimi, Mimo, GLM, DeepSeek, Grok on Dek Go |
| Anthropic messages      | ✅ Native — `tool_result.content: AnthropicContentBlock[]`    | Used by MiniMax M2.5/M2.7, Qwen3.5/3.6/3.7-max         |
| Google Gemini           | ✅ Native — `functionResponse.response.parts: [{inlineData}]` | Used by Gemini family (when available on Dek)     |
...
- Manual test with `chrome-devtools-mcp` + Kimi K2.7 Code on Dek Go:
