...
**Issues:** [#94](https://github.com/1000bath/dek-copilot-chat/issues/94), [#38](https://github.com/1000bath/dek-copilot-chat/issues/38) (superseded)
...
Top-level and tool-result image attachments are now resized and re-encoded to provider-safe dimensions **before** the final payload guard runs. This mirrors the normalization that the standalone Dek CLI applies before forwarding images to the gateway, fixing the long-standing class of `400 Upstream request failed` errors that hit images which were valid pixels but exceeded the gateway's implicit size contract.
