...
This document records the per-model Thinking controls feature for Dek Go and Dek Zen models in GitHub Copilot Chat.

The original session started on **2026-05-17 Asia/Jakarta**. The feature request was to expose model-family-specific Thinking behavior similar to Copilot's built-in `Thinking Effort` picker, while preserving separate Dek Go and Dek Zen provider configuration.

Before this work, the extension only had `dekgo.debugReasoning`, which writes provider `reasoning_content` to the Dek output channel for diagnostics. That setting remains diagnostic-only and is separate from user-facing Thinking controls.

---

## Problem

Dek models do not all use the same request fields for reasoning or thinking. A single global on/off setting cannot accurately represent the available controls.
...
The feature adds explicit settings under the `dekgo.thinking.*` namespace:

| Setting                          | Purpose                       |
| -------------------------------- | ----------------------------- |
| `dekgo.thinking.deepseek`   | DeepSeek reasoning effort     |
| `dekgo.thinking.glm`        | GLM on/off thinking           |
| `dekgo.thinking.kimi`       | Kimi on/off thinking          |
| `dekgo.thinking.minimax`    | MiniMax on/off thinking       |
| `dekgo.thinking.mimo`       | Mimo reasoning effort         |
| `dekgo.thinking.qwen`       | Qwen auto/on/off thinking     |
| `dekgo.thinking.qwenBudget` | Optional Qwen thinking budget |
...
When `models.dev` provides `reasoning_options`, those options are preferred over hardcoded family defaults. This keeps the picker aligned with upstream model metadata when Dek updates supported reasoning levels.
...
| `buildThinkingPayload()`              | Emits Dek request fields for each model family             |
...
| Native picker submenu is unavailable     | Users can configure defaults via `Dek: Set Thinking Effort…` or Settings    |
