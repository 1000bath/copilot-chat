...
**GitHub Issue:** [1000bath/dek-copilot-chat#24](https://github.com/1000bath/dek-copilot-chat/issues/24) (Zen HTTP 400 errors)
**GitHub Issue:** [1000bath/dek-copilot-chat#25](https://github.com/1000bath/dek-copilot-chat/issues/25) (Kimi K2.7 constraints)
**GitHub PR:** [#46](https://github.com/1000bath/dek-copilot-chat/pull/46) (Kimi K2.7 fix)
**GitHub PR:** [#107](https://github.com/1000bath/dek-copilot-chat/pull/107) (Transient 5xx retry, by [@Fahad090NP](https://github.com/Fahad090NP))
...
The upstream Dek API serves 30+ models from different providers (Moonshot, DeepSeek, ZhipuAI, Alibaba, etc.). Each provider has its own API contract for parameters like `thinking`, `temperature`, and `reasoning_effort`. These contracts change over time, but the extension's metadata cache (models.dev) only refreshes every 6 hours.
...
A standalone Node.js script that tests all models against the Dek API before release.
...
| --------------- | ------------------- | ----------------------------------------------------------------- |
| `--api-key`     | `$OPENCODE_API_KEY` | API key for Dek                                              |
| `--go`          | `true`              | Include Dek Go models                                        |
...
- Mock server simulates Dek API behavior
...
- Reuses extension's exact logic (`buildThinkingPayload`, `resolveModelRouting`, `buildDekGatewayAuthHeaders`)
- Tests ALL thinking/reasoning parameter combinations for each model
- 18 models (13 Go + 5 Zen free) validated against live Dek API
