...
**Issues:** [#92](https://github.com/1000bath/dek-copilot-chat/issues/92), [#87](https://github.com/1000bath/dek-copilot-chat/issues/87)
...
Models appeared in the picker as `Dek Go / DeepSeek V4 Flash`. Copilot already renders the provider separately for each row, so the prefix in the model name was redundant. In a narrow chat view the selected model name was truncated before the actual model identifier, forcing users to widen the chat pane to roughly one-third of the screen just to read which model was active.

### Solution

New setting `dekgo.showProviderPrefix` (default `true`, preserving existing behavior). When set to `false`, model names display only the formatted model name without the `Dek Go` / `Dek Zen` prefix.

```json
"dekgo.showProviderPrefix": false
```
...
- The configuration-change listener in `activate()` detects `dekgo.showProviderPrefix` and calls `notifyModelInfoChanged()` on every provider in the `modelInfoProviders` array.
...
| `package.json` | New setting `dekgo.showProviderPrefix` |
