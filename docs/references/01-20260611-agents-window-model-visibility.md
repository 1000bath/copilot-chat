...
# Agents Window Model Visibility — Dek Models in VS Code Agents Tab
...
GitHub Issue #11 requests: _"Show Dek models in the VS Code Agents window model picker."_

The user wants Dek models to appear when selecting a model in the Agents tab, not just in the Chat view. Additionally, the user initially wanted a dedicated "Dek" tab in the Agents window (not appearing under "Copilot CLI").
...
- Our extension uses vendors `dekgo` and `dekzen`
...
Example: `dekgo/deepseek-v4-flash`
...
"contributes": {
    "chatSessions": [{
        ...
        "name": "opencode",
        "displayName": "Dek",
        "description": "AI coding agent powered by Dek",
...
| `src/extension.ts`                      | `DekModel` interface at ~line 180 — extends `vscode.LanguageModelChatInformation` |
...
| `src/providerTypes.ts`                  | Exports `GO_VENDOR = "dekgo"` and `ZEN_VENDOR = "dekzen"`                    |
...
_Research session: 2026-06-11 | Paired with GitHub Issue [#11](https://github.com/1000bath/dek-copilot-chat/issues/11)_
