...
```text
Dek Go API request failed (400) model=gpt-5.6-luna payloadBytes=464399:
...
- ⏳ Live gateway: attach an image to `gpt-5.6-luna` in Copilot Chat and confirm the
  model answers about the image (requires the user's Dek Go API key). Also spot-
...
(GPT-5.x family via Dek Go) was affected by the same malformed image shape. The
