...
**GitHub Issue:** [#25](https://github.com/1000bath/dek-copilot-chat/issues/25)  
...
The K2.7-code API spec does **not** list `temperature` as a supported body parameter (unlike K2.6 which accepts it). The error message from the Dek Go gateway — _"invalid temperature: only 1 is allowed for this model"_ — confirms the upstream Moonshot API rejects any non-default temperature for K2.7-code.
...
- Default setting `dekgo.thinking.kimi` is `"off"` (see `getSettings`, line ~2916)
...
1. Install extension with default settings (`dekgo.thinking.kimi: "off"`)
...
- Update `package.json` configuration description for `dekgo.thinking.kimi` to note that K2.7 ignores this setting (always on).
...
- [x] **`kimi-k2.7-code-highspeed`** — Not yet listed on Dek Go. The `/^kimi-k2\.7/i` regex in `buildThinkingPayload` and `buildFamilyThinkingSchema` already covers it if/when it ships.
...
- GitHub issue: <https://github.com/1000bath/dek-copilot-chat/issues/25>
