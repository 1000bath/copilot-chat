...
This document records the complete 2026-05-17 session that fixed incorrect Dek Zen model labels in the VS Code and Copilot Chat model pickers.

The visible bug was that upstream model IDs with version-like numeric suffixes were rendered with spaces instead of decimal dots. For example, `claude-opus-4-6` appeared as `Dek Zen / Claude Opus 4 6` instead of `Dek Zen / Claude Opus 4.6`.
...
Dek Zen model names in the model picker were not preserving version decimals for IDs encoded with hyphen-separated numeric parts.
...
**Input:** The user shared screenshots showing Dek Zen rows such as:

```text
Dek Zen / Claude Haiku 4 5
Dek Zen / Claude Opus 4 1
Dek Zen / Claude Opus 4 5
Dek Zen / Claude Opus 4 6
Dek Zen / Claude Sonnet 4 5
Dek Zen / Claude Sonnet 4 6
```
...
```bash
rg -n "Dek Zen|opencode zen|zen|model|models|naming|displayName|display_name|label" .
...
```bash
unzip -p dek-copilot-chat-0.1.4.vsix extension/out/extension.js | rg -n "MODEL_METADATA_REVISION|formatModelName|thinking-2026|naming-2026"
unzip -p dek-copilot-chat-dev.vsix extension/out/extension.js | rg -n "MODEL_METADATA_REVISION|formatModelName|thinking-2026|naming-2026"
```
...
```text
dek-copilot-chat-0.1.5.vsix
```

The package contents were verified:

```bash
unzip -p dek-copilot-chat-0.1.5.vsix extension/out/extension.js | rg -n "naming-2026|versionParts|formatModelName"
```
...
```text
~/.vscode/extensions/1000bath.dek-copilot-chat-0.1.5
```
...
| `dek-copilot-chat-0.1.4.vsix` | Rebuilt with the naming fix and new metadata revision                                |

**Artifact verification:**

```bash
unzip -p dek-copilot-chat-0.1.4.vsix extension/package.json | rg -n '"version"'
unzip -p dek-copilot-chat-0.1.4.vsix extension/out/extension.js | rg -n "naming-2026|versionParts|formatModelName"
```
...
| `dek-copilot-chat-0.1.4.vsix` | Historical rebuilt local package containing the `0.1.4` naming fix                |
