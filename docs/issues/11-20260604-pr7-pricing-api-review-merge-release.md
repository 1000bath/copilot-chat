...
External contributor [@Wallacy](https://github.com/Wallacy) submitted PR [#7](https://github.com/1000bath/dek-copilot-chat/pull/7) to add support for VS Code's proposed `languageModelPricing` API. This session covered the full review cycle: code analysis, risk assessment, approving review with feedback, merge, local VSIX build, version bump to 0.1.8, and amend + force push.

---

## Problem

VS Code recently added a `vscode.proposed.languageModelPricing` API that exposes `pricing`, `inputCost`, `outputCost`, `cacheCost`, and `priceCategory` on every registered language model. The Dek Go/Zen providers were not populating those fields. The `models.dev` registry already publishes per-model cost data — we just weren't reading it.

Additionally, the `dekgo.experimentalContextIndicator` configuration was redundant after commit `ca8bbb6` which implemented the same capability natively.
...
4. **Redundant experimental config** — `dekgo.experimentalContextIndicator` and its hook bridge infrastructure were no longer needed.
...
- `npm run package` — produced `dek-copilot-chat-0.1.8.vsix` (91.18 KB, 32 files).
...
# Package
npm run package    # dek-copilot-chat-0.1.8.vsix (91.18 KB)

# Install
open -a "Visual Studio Code" dek-copilot-chat-0.1.8.vsix
