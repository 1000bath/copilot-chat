...
Dek Go API proxy returns HTTP 500 "Internal server error" when the JSON request body exceeds ~400 KB. Long chat sessions accumulate message history and tool definitions that push past this limit, even though the model's token context window (e.g. 1M tokens for deepseek-v4-pro) is far from full. This document traces the investigation, multiple fix iterations, and final solution.

> **Note:** Gzip compression was later removed in v0.2.6 because the Dek Go/Zen proxy does not support gzip request bodies. See devlog entry for v0.2.6 Payload Simplification.
...
**Root Cause:** Dek Go API proxy has an internal HTTP body size limit of ~400 KB. No guard existed in the extension.
...
Gzip compression was the architecturally correct fix (transport-layer solution for a transport-layer problem), but was later removed in **v0.2.6** because the Dek Go/Zen proxy does not support gzip-encoded request bodies.
