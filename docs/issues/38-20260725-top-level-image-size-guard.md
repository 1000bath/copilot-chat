...
Dek CLI contract (2000×2000 / 5 MB base64) **before** any guard runs, so
...
into Copilot Chat while using a vision-capable Dek model (e.g. `mimo-v2.5`),
the request failed with:

```text
Dek Go API request failed (400) model=mimo-v2.5 payloadBytes=3182845:
Error from provider (Console Go): Upstream request failed
```

The 3.18 MB payload (~2.4 MB raw image × 1.33 base64 overhead) was forwarded
directly to the Dek Go gateway, which rejected it. Top-level image
...
Reason: Dek Go API request failed (400) model=mimo-v2.5 payloadBytes=3182845:
Error from provider (Console Go): Upstream request failed
  at buildDekRequestError (.../out/errors.js:39:12)
  at streamDekResponse (.../out/streaming.js:333:73)
...
4. A 3.18 MB payload exceeds the (unpublished) Dek Go gateway limit and is
   rejected upstream.
...
| **Dek Go/Zen** | Not published                             | Not published           | [opencode.ai/docs/zen](https://opencode.ai/docs/zen)                                                                           |
...
Dek Go gateway. Combined with Anthropic's published 5–10 MB per-image
...
- Dek Go verified to reject 3.18 MB; 2 MB raw → ~2.7 MB base64 stays under
...
2. **Single threshold for all transports.** The 2 MB cap is the most
   conservative across Anthropic (5–10 MB) and observed Dek Go behavior
