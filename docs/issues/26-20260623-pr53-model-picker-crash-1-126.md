...
VS Code 1.126 shipped a **unified model picker** (internal PR #321026) that changed how `LanguageModelChatProvider` implementations are resolved and displayed. Two regressions hit Dek providers on 1.126:

1. **Hard crash** the moment the model picker tried to read Dek model metadata.
2. **Duplicate entries** in the picker for every Dek model.
...
**Fixed in:** v0.3.4 (PR [#53](https://github.com/1000bath/dek-copilot-chat/pull/53))  
**Issue report:** [#51](https://github.com/1000bath/dek-copilot-chat/issues/51)
...
Because Dek declares a `configuration` schema (`apiKey`), every model acquired a second cache identity (`vendor/group/modelId` versus `vendor/modelId`) and appeared twice in the picker.
...
| Removed                                                                                   | Reason                                                                                                                 |
| ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `category` object field from `DekModel` and the returned model info                  | Type mismatch with `LanguageModelChatInformation`.                                                                     |
...
| `onLanguageModelChatProvider:dekgo-agent` and `:dekzen-agent` activation events | Not needed with the new fallback strategy.                                                                             |
