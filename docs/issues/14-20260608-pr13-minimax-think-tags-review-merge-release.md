...
Related issue: [#12 — Minimax M3 `<think />` blocks are displayed in the response](https://github.com/1000bath/dek-copilot-chat/issues/12)

---

## Root Cause

The Dek Go/Zen proxy passes through model output as-is. MiniMax's API choice to embed reasoning in `content` rather than a separate field meant the extension's streaming extractors (`OpenAiResponseExtractor`, `AnthropicResponseExtractor`) treated the entire content — including `অসমীয়া` tags — as regular text output.
...
| `dekgo.stripThinkTags` setting                  | New VS Code configuration with `"auto"` default                                                       |
...
| P5  | `dekgo.stripThinkTags` setting                    | `package.json`     | `"never"` / `"auto"` / `"always"` with `"auto"` default                                 |
...
2. **Config-gated** — `dekgo.stripThinkTags` defaults to `"auto"` (apply only to known models). Set to `"always"` to strip for every model, or `"never"` to disable.
...
```bash
vsce package --no-dependencies    # dek-copilot-chat-0.2.2.vsix (108.76 KB)
"/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code" \
  --install-extension dek-copilot-chat-0.2.2.vsix --force
...
- `package.json` — New `dekgo.stripThinkTags` setting
