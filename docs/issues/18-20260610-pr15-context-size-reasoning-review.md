...
**PR:** [1000bath/dek-copilot-chat#15](https://github.com/1000bath/dek-copilot-chat/pull/15)  
...
| P15 | `dekgo.thinking.mimo` setting                                          | `package.json`     | New setting: `off` / `low` / `medium` / `high`                               |
| P16 | `dekgo.thinking.minimax` setting                                       | `package.json`     | New setting: `off` / `on`                                                    |
| P17 | `dekgo.thinking.deepseek` expanded                                     | `package.json`     | Added `low` / `medium` to existing `off` / `high` / `max`                    |
...
| 3   | **MiniMax differentiation** | Correctly maps `minimax-m3` → `adaptive` vs `minimax-m2.*` → `enabled`, matching upstream Dek `transform.ts`               |
...
`MODEL_METADATA_CACHE_KEY` changed from `"dekgo.modelMetadataCache.v4"` → `"opencode.modelMetadataCache.v5"` — note also the prefix change from `dekgo` to `opencode`.

### Settings Added

| Setting                        | Type     | Options                               | Default |
| ------------------------------ | -------- | ------------------------------------- | ------- |
| `dekgo.thinking.mimo`     | `string` | `off`, `low`, `medium`, `high`        | `off`   |
| `dekgo.thinking.minimax`  | `string` | `off`, `on`                           | `off`   |
| `dekgo.thinking.deepseek` | `string` | `off`, `low`, `medium`, `high`, `max` | `off`   |
