...
This document records the full **2026-05-20 Asia/Jakarta** debugging and release-preparation session for image attachments in VS Code Copilot Chat when using Dek models.

The session had four separate findings:

1. Image requests initially failed inside the extension before reaching Dek because image bytes were encoded with a spread call that overflowed the JavaScript call stack.
2. After the encoding fix, requests reached Dek successfully, but `qwen3.5-plus` vision requests could still fail with provider-side Alibaba `429 insufficient_quota` when the request also forced a large Qwen `thinking_budget`.
3. `glm-5`, `glm-5.1`, MiniMax M2, and some MiMo Pro rows were incorrectly advertised to VS Code as `Vision` even though Dek metadata did not support image attachments for those model IDs.
...
```text
convertMessage (.../.vscode/extensions/1000bath.dek-copilot-chat-0.1.4/out/extension.js:830:40)
DekProvider.provideLanguageModelChatResponse (.../out/extension.js:415:56)
```
...
This spreads every image byte as a separate function argument. For sufficiently large image attachments, JavaScript exceeds the maximum call stack / argument limit before the request can be sent to Dek.
...
"/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code" \
  --install-extension dek-copilot-chat-0.1.5.vsix \
  --force
```
...
```bash
"/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code" \
  --list-extensions --show-versions | rg 'ltmoerdani\\.dek-copilot-chat'
```

Expected installed version:

```text
1000bath.dek-copilot-chat@0.1.5
```
...
The image encoding fix worked. The request now reached Dek and the Alibaba-backed provider. The remaining failure was not a stack overflow or request conversion bug; it was provider-side quota/token pressure.
...
"/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code" \
  --install-extension dek-copilot-chat-0.1.6.vsix \
  --force
```
...
```text
1000bath.dek-copilot-chat@0.1.6
```
...
```text
Request: model=dekgo:glm-5::naming-2026-05-17-a rawModel=glm-5 endpoint=chat-completions messages=4 images=yes ...
payloadBytes=606802
```
...
`models.dev` metadata was queried for Dek model capabilities. For `glm-5` and `glm-5.1`, the Dek metadata showed:
...
That made VS Code advertise a `Vision` capability for GLM rows even though Dek marked both models as text-only. Copilot could therefore accept image attachments, but the model could not inspect them.
...
A model should be advertised as image-capable only when the relevant Dek provider metadata has both:
...
| Model               | Reason                                                      |
| ------------------- | ----------------------------------------------------------- |
| `kimi-k2.5`         | Dek Go metadata supports image attachment              |
| `kimi-k2.6`         | Dek Go metadata supports image attachment              |
| `mimo-v2.5`         | Dek Go metadata supports image attachment              |
| `mimo-v2-omni`      | Dek Go metadata supports image attachment              |
| `qwen3.5-plus`      | Dek Go metadata supports image attachment              |
| `qwen3.6-plus`      | Dek Go/Dek Zen metadata supports image attachment |
| `qwen3.6-plus-free` | Dek Zen metadata supports image attachment             |
...
| `minimax-m2.5`      | Dek metadata `attachment: false`, input only `text`                                      |
| `minimax-m2.7`      | Dek metadata `attachment: false`, input only `text`                                      |
| `minimax-m2.5-free` | Dek Zen metadata `attachment: false`, input only `text`                                  |
| `mimo-v2-pro`       | Dek Go reported `attachment: true` but input modality only `text`; not valid image input |
| `mimo-v2.5-pro`     | Dek Go reported `attachment: true` but input modality only `text`; not valid image input |
...
PATH=/opt/homebrew/opt/node/bin:$PATH npm run package
"/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code" \
  --install-extension dek-copilot-chat-0.1.7.vsix \
  --force
```
...
```bash
"/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code" \
  --locate-extension 1000bath.dek-copilot-chat
```

Expected path:

```text
/Users/ltmoerdani/.vscode/extensions/1000bath.dek-copilot-chat-0.1.7
```
...
```text
> dek-copilot-chat@0.1.5 compile
> tsc -p ./
```
...
| GLM accepted image payloads but could not inspect them               | Extension advertised `glm-5` / `glm-5.1` as vision-capable despite Dek metadata `attachment: false` and input only `text` | Remove GLM from `VISION_CAPABLE_MODELS`                                             | ✅ Solved    |
| Some MiniMax/MiMo Pro rows incorrectly showed `Vision`               | Metadata either had `attachment: false` or no `"image"` input modality                                                         | Keep `Vision` only when Dek metadata supports image attachment and image input | ✅ Solved    |
