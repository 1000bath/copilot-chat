...
**GitHub Issues:** [#22](https://github.com/1000bath/dek-copilot-chat/issues/22), [#71](https://github.com/1000bath/dek-copilot-chat/issues/71)  
...
Issues **#22** (`chat.agent.thinkingStyle` not respected) and **#71** (thinking tokens not displaying) are **the same bug**: reasoning content from Dek models is never emitted to the VS Code Chat UI as a thinking part, so there is nothing for `chat.agent.thinkingStyle` to style.
...
| **Reported on** | Any Dek model with reasoning                                                              | Any Dek Go/Zen model                                                        |
...
This fix is **independent** of `dekgo.stripThinkTags`:
...
- `dekgo.stripThinkTags` setting — untouched. Think-tag filtering composes with the new thinking part surfacing.
