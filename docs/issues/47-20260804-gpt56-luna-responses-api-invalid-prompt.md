...
**GitHub:** [#103](https://github.com/1000bath/dek-copilot-chat/issues/103)
...
```text
Dek Go API request failed (400) model=gpt-5.6-luna payloadBytes=1528273:
...
`text.verbosity` is a native OpenAI Responses API field. The Dek Go gateway is a proxy in front of other providers, and there is no guarantee it understands or forwards this field. A provider that rejects unknown fields will return `invalid_prompt`.
...
The `text` field was removed from the shared Responses request envelope. It is non-essential, and avoiding it keeps the Dek proxy compatible with upstream providers that reject unknown or unsupported options.
