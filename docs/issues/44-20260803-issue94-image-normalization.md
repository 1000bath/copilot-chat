**Status:** ✅ Solved — merged via [PR #102](https://github.com/1000bath/dek-copilot-chat/pull/102) (commit `a273a1f`, follow-up `4572a9f`). Shipped in 0.5.0.

# Image Attachment Normalization (#94)

Dek Go rejected some image attachments even though the same model and
image worked in the Dek CLI. The extension sent the original bytes as an
...
used by Dek: a maximum `2000x2000` image size and a `5 MB` base64 payload.
