...
This document covers **Phase 1** of the 2026-05-17 session: implementing per-model Thinking mode configuration for the Dek Go and Dek Zen VS Code Language Model Chat providers.
...
The feature request: Dek Go and Zen models should expose model-family-specific Thinking controls similar to Copilot's built-in `Thinking Effort` picker. The user wanted to configure reasoning behavior per model family instead of relying only on the diagnostic `dekgo.debugReasoning` setting.

---

## Problem

The extension supported reasoning-related diagnostics through `dekgo.debugReasoning`, which writes provider `reasoning_content` to the Dek output channel. That setting is useful for debugging, but it is not a user-facing Thinking mode control.
...
| Setting                         | Purpose              |
| ------------------------------- | -------------------- |
| `dekgo.deepSeekThinking`   | DeepSeek family mode |
| `dekgo.glmThinking`        | GLM family mode      |
| `dekgo.kimiThinking`       | Kimi family mode     |
| `dekgo.qwenThinking`       | Qwen family mode     |
| `dekgo.qwenThinkingBudget` | Optional Qwen budget |
...
**Result:** VS Code began showing model configuration labels such as `Off` and `Auto` for Dek models. This proved that `configurationSchema` metadata was being read by the model picker.
...
```text
Dek Zen / Qwen3.6 Plus · Auto
Dek Zen / Qwen3.6 Plus · Off
Dek Zen / Qwen3.6 Plus · On
Dek Zen / Kimi K2.6 · Off
Dek Zen / Kimi K2.6 · On
```
...
| Request payload     | `src/extension.ts`                      | Maps Thinking settings to Dek request fields             |
| User fallback       | `src/extension.ts`                      | Adds `Dek: Set Thinking Effort…` command                 |
...
| `buildThinkingPayload()`              | Emits the correct Dek API fields                                                    |
...
The extension had reasoning diagnostics (`dekgo.debugReasoning`) but did not expose user-facing Thinking controls per model family. This document covers the implementation of those controls.
...
This prevents stale assumptions when Dek updates a model's supported reasoning levels.
...
| Native submenu not exposed                      | User can use `Dek: Set Thinking Effort…` or family settings                     |
...
- No Dek API keys, GitHub tokens, or other secrets were added to documentation.
