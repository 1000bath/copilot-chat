...
This document records the full 2026-05-27 Asia/Jakarta session for making the VS Code Copilot Chat **Context Window** indicator work with Dek BYOK models.
...
but it did not move during long conversations when Dek Go or Zen models were selected.

Users reported:

- Qwen 3.6 via Dek Go subscription had long chat sessions but the Context Window UI stayed at `0%`.
...
PR #6 added Dek-specific usage telemetry and an experimental context hook, but its initial usage DataPart MIME was custom:

```ts
application / vnd.opencode.usage + json;
```

That MIME is useful for Dek internal diagnostics, but it is not the native Copilot Context Window usage channel. The final integration therefore emits both:

| MIME                                  | Consumer                                                       |
| ------------------------------------- | -------------------------------------------------------------- |
| `usage`                               | VS Code / Copilot Chat Context Window                          |
| `application/vnd.opencode.usage+json` | Dek-specific diagnostics and future internal integrations |
...
"/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code" \
  --install-extension /Users/ltmoerdani/Startup/dek-copilot-chat/dek-copilot-chat-0.1.7.vsix \
  --force
```
...
rg -n "COPILOT_USAGE_DATA_MIME|application/vnd.opencode.usage|stream_options|include_usage|estimateChatMessageTokenCount" \
  "$HOME/.vscode/extensions/1000bath.dek-copilot-chat-0.1.7/out"
```
...
| Dek internal telemetry   | Not separate                            | Emits `application/vnd.opencode.usage+json` | Keeps custom MIME           |
...
Users reported that the Context Window indicator did not move for long sessions, including Qwen 3.6 on Dek Go. Wallacy noted PR #6 had mixed results across models.
...
- Model ID
- Provider (`Dek Go` or `Dek Zen`)
- VS Code version
- Copilot Chat version if available
- Dek output logs with secrets removed
...
| File                                | Change                                                                                                                |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `src/chatParts.ts`                  | Added usage DataPart helpers and dual MIME reporting (`usage` + Dek custom MIME)                                 |
...
| `src/openCodeAuth.ts`               | Added Dek gateway auth header builder for messages/google/bearer routes                                          |
...
```text
1000bath.dek-copilot-chat@0.1.7
```
...
5. **Dek diagnostics channel:** the same normalized usage payload is emitted with the Dek custom MIME for internal diagnostics.
...
This design makes the native VS Code path the primary compatibility layer and keeps Dek-specific diagnostics as an additive feature.
