...
**GitHub Issue:** [#22](https://github.com/1000bath/dek-copilot-chat/issues/22)  
...
**Observed bug:** Reasoning content from Dek models (DeepSeek, Qwen, GLM, Kimi, MiniMax, MiMo) is **always shown fully expanded as plain text**, regardless of the `chat.agent.thinkingStyle` setting.

This is **NOT** the same problem addressed by `dekgo.stripThinkTags`. That setting only removes `<think>` tags from MiniMax M3 output. The user's request here is to have reasoning **surfaced through the proper thinking channel** so that VS Code Chat UI can apply `chat.agent.thinkingStyle`.
...
| `enabledApiProposalNames` / `enabledApiProposals` in `package.json`                   | ❌ **Empty** — proposed API is activated implicitly via `activationEvents: onLanguageModelChatProvider:dekgo` (lines 59-60), not via an explicit proposal declaration |
...
- **`dekgo.stripThinkTags`** setting — values: `"auto"` (default, MiniMax M3 only), `"always"`, `"never"`.
...
- **Cross-link:** This repo's PR [#38](https://github.com/1000bath/dek-copilot-chat/pull/38) ("fix: add option to turn off reasoning") is linked from the upstream issue, but PR #38 only addresses turning reasoning **off** for models.dev effort-only schemas — it does not solve the surfacing problem.

Until the VS Code team finalizes and ships a thinking part API for BYOK providers (proposed `chatProvider` API with a concrete `LanguageModelThinkingPart` definition), there is **no extension-side fix** that will make `chat.agent.thinkingStyle` work for Dek BYOK models.
