...
This document records the full session that investigated stale Dek model entries and implemented filtering so unavailable or deprecated models do not appear in the VS Code Copilot Chat model picker.
...
- `ring-2.6-1t`, which should not be surfaced by this extension when it cannot be accessed through the configured Dek catalog

The final approach was not just to delete two strings. The extension now combines:

- live Dek `/models` discovery
...
```text
Dek Go API request failed (400) model=trinity-large-preview-free:
Error from provider: No endpoints found for arcee-ai/trinity-large-preview:free.
```

```text
Dek Go API request failed (400) model=ring-2.6-1t-free:
...
- `fetchModels()` queries the Dek `/models` endpoint
...
- Zen can be filtered to free models via `dekgo.freeOnly`
...
The live Dek endpoints and `models.dev` registry were checked.

Findings:

| Source                          | Finding                                                                                |
| ------------------------------- | -------------------------------------------------------------------------------------- |
| Dek Go `/zen/go/v1/models` | `ring-2.6-1t` was not returned in the active Go list                                   |
| Dek Zen `/zen/v1/models`   | `ring-2.6-1t-free` and `trinity-large-preview-free` could still appear in the catalog  |
| `models.dev/api.json`           | `ring-2.6-1t-free` and `trinity-large-preview-free` were marked `status: "deprecated"` |

The important discovery was that Dek `/models` is not a sufficient serving guarantee. A model can still be listed in the catalog while the underlying provider returns a 404 because no endpoint exists or the free tier was withdrawn.
...
- the Dek gateway wrapped provider 404s as HTTP 400 responses
- the local error message could misleadingly say `Dek Go API request failed` even when the selected model came from Zen
...
| Root Cause                  | Detail                                                                                                                  |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Catalog drift               | Dek `/models` could still list models that no longer had a usable provider endpoint                                |
...
There was also a diagnostics issue: the transport error text could use the wrong provider display name, making Zen failures appear as Dek Go failures.
...
1. Fetch live model IDs from Dek.
