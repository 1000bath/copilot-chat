# Issue #43 — Non-agent Dek Zen returns 0 models (API key via extension command)

**Date:** 2026-08-03
**Status:** ✅ Resolved
**Related:** GitHub Issue [#86](https://github.com/1000bath/dek-copilot-chat/issues/86)
**Reporter:** [@Witchcraft2k](https://github.com/Witchcraft2k)
**Extension version affected:** 0.4.3
**Fixed in:** [0.5.0](https://github.com/1000bath/dek-copilot-chat/releases/tag/v0.5.0) (PR [#101](https://github.com/1000bath/dek-copilot-chat/pull/101), merged 2026-08-03)

## Problem

When the Dek Zen API key is stored via the extension's own command (`Dek Go: Set API Key`) — rather than through VS Code's native Language Models → Add Models BYOK flow — the non-agent `dekzen` provider returns **0 models** in `vscode.lm.selectChatModels({ vendor: "dekzen" })`. The Zen free models never appear in the Chat model picker dropdown.

### Symptom matrix (reported)

| Provider                      | Models visible | Expected |
| ----------------------------- | -------------- | -------- |
| `dekgo` (non-agent Go)   | ✅ 23          | 23       |
| `dekgo-agent`            | ✅ 23          | 23       |
| `dekzen-agent`           | ✅ 7           | 7        |
| `dekzen` (non-agent Zen) | ❌ **0**       | 7        |

### Steps to reproduce

1. Install the extension.
2. Set the Dek API key via the command `Dek Go: Set API Key` (NOT via the native VS Code Language Models → Add Models BYOK flow).
3. Run `Dek: Model Picker Diagnostics` — observe `vendor: dekzen` shows `models: 0`.
4. Run `Dek Zen: Diagnostics` — observe `Models visible through vscode.lm.selectChatModels({ vendor: "dekzen" }): 0`.

## Root Cause (Evidence-Based)

### Location

`src/extension.ts`, `DekLMChatProvider.provideLanguageModelChatInformation()` — the API-key fallback guard:
...
- GitHub Issue: [#86](https://github.com/1000bath/dek-copilot-chat/issues/86)
- Pull Request: [#101](https://github.com/1000bath/dek-copilot-chat/pull/101) (merged via merge commit `40e5db5`)
- Regression follow-up: [#106](https://github.com/1000bath/dek-copilot-chat/issues/106) / PR [#108](https://github.com/1000bath/dek-copilot-chat/pull/108)
