# PR #113 — Harden Dek VS Code Bridge

**Date:** 2026-08-07
**Status:** ✅ Merged (merge commit `268059f`, 2026-08-07T15:30:35Z)
**Related:** PR [#113](https://github.com/1000bath/dek-copilot-chat/pull/113)
...
Major hardening of the Dek language-model bridge against four classes of failure that surfaced once multi-turn agent sessions, Responses-routed models, and long tool-heavy prompts hit production:
...
- Add provider management commands and explicit utility-model configuration (`Dek: Configure Utility Models`).
...
- `opencode serve` is documented but not registered as a native VS Code model source because its session/agent API is not a compatible inference endpoint for VS Code's tool loop. See [Dek server documentation](https://opencode.ai/docs/server/) and #88.
...
- PR: [#113](https://github.com/1000bath/dek-copilot-chat/pull/113)
