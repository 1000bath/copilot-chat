...
> **GitHub issue:** [#78 — `[BUG] Could not fetch Dek Zen (Agents) model list. Using bundled model list. fetch failed`](https://github.com/1000bath/dek-copilot-chat/issues/78)
...
```text
Could not fetch Dek Go model list. Using bundled model list. fetch failed
Could not fetch Dek Zen (Agents) model list. Using bundled model list. fetch failed
```
...
| Extension  | `1000bath.dek-copilot-chat` 0.4.1                                                            |
| Symptom    | Both `dekgo` and `dekzen` providers fail to fetch; Zen list briefly flashes then vanishes |
...
[`#51`](https://github.com/1000bath/dek-copilot-chat/issues/51) shared the same log line ("Could not fetch … fetch failed") but the actual root cause was a `TypeError: Cannot read properties of undefined (reading 'charAt')` from VS Code 1.126's unified picker passing `category` as a plain string instead of `{ label, order }`. PR #53 fixed that schema mismatch.
...
3. **No `User-Agent`** — strict gateways can silently drop anonymous requests. (`refreshDekModelMetadata()` already sends one via the gateway-header builder, but `fetchModels` did not.)
4. **Stale `OPEN_CODE_USER_AGENT` constant** — hardcoded `"dek-copilot-chat/0.3.6 VSCode"` while `package.json` was at `0.4.1` (drift, a recurring problem: see `docs/issues/17`).
...
After the initial fix landed, the issue reporter (`@leiyu1980`) replied that they could not find `Dek Go: Refresh Models` in the Command Palette. Investigation revealed this was a UX gap, not a bug:

### What was wrong

- `Refresh Models` existed only as an **action inside the `Dek Go: Manage Provider` QuickPick** (`src/extension.ts` `manage()`). It was never registered as a top-level command.
- **Zen had no `Manage Provider` command at all** — only `Dek Zen: Diagnostics`. So a Zen user (which is the reporter's case: "Zen models flash briefly before disappearing") had zero manual refresh path via the palette.
- The README commands table only listed `Manage Provider` for Go, reinforcing the asymmetry.

The maintainer's first reply had told the reporter to "run `Dek Go: Refresh Models` from the Command Palette", which was wrong. The command didn't exist at that name.
...
| Command                         | Behavior                                                                                                                                                                                    |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Dek Go: Refresh Models`   | Skips the Manage Provider QuickPick. Goes straight to a fresh model-list fetch + `changeEmitter.fire()` so VS Code re-resolves the picker. Falls back to `setApiKey()` if no key is stored. |
| `Dek Zen: Manage Provider` | Parity with Go. Opens the same QuickPick (Set / Clear / Test / Refresh).                                                                                                                    |
| `Dek Zen: Refresh Models`  | Same as the Go refresh command, scoped to Zen.                                                                                                                                              |

Implementation: a new public `refreshModels()` method on `DekProvider` wraps the private `refreshMetadataAndModels()` + `changeEmitter.fire()` + toast. `manage()`'s "Refresh Models" action now delegates to this method (single source of truth). No change to existing `dekgo.manage` behavior — backward compatible.
...
- GitHub issue: [#78](https://github.com/1000bath/dek-copilot-chat/issues/78)
- Related (closed, different root cause): [#51](https://github.com/1000bath/dek-copilot-chat/issues/51) → PR [#53](https://github.com/1000bath/dek-copilot-chat/pull/53)
...
- Dek Zen gateway endpoints: [`opencode.ai/docs/zen`](https://opencode.ai/docs/zen/) and [`opencode.ai/docs/go`](https://opencode.ai/docs/go/)
- Prior drift incident (`OPEN_CODE_USER_AGENT`): `docs/issues/17-20260609-project-cleanup-immediate-bugfixes.md`
- Pattern reference (timeout already in use): `refreshDekModelMetadata()` in `src/extension.ts`
