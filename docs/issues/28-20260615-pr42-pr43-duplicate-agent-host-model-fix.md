...
**GitHub Issue:** [1000bath/dek-copilot-chat#41](https://github.com/1000bath/dek-copilot-chat/issues/41)  
**GitHub PR:** [#42](https://github.com/1000bath/dek-copilot-chat/pull/42) (by [@Marinski](https://github.com/Marinski)) — opt-in gate hotfix  
**GitHub PR:** [#43](https://github.com/1000bath/dek-copilot-chat/pull/43) (by [@Wallacy](https://github.com/Wallacy)) — separate vendor IDs (final solution)  
...
```text
dekgo:qwen3.7-max::session-2026-05-21-b
dekgo:qwen3.7-max::session-2026-05-21-b::agent-host
```
...
| `package.json`     | Add `dekgo.showInAgentsWindow` boolean setting (default `false`)                                                    |
...
Register agent models under **dedicated vendor IDs** (`dekgo-agent`, `dekzen-agent`) so each vendor group shows only its models. Eliminates duplication entirely across all surfaces.

### Vendor Architecture

| Vendor              | Models                                                    | Visible in                                      |
| ------------------- | --------------------------------------------------------- | ----------------------------------------------- |
| `dekgo`        | General models                                            | Chat view, Manage panel                         |
| `dekgo-agent`  | Agent-only models (`targetChatSessionType: "copilotcli"`) | Agents window, Manage panel (hidden by default) |
| `dekzen`       | General models                                            | Chat view, Manage panel                         |
| `dekzen-agent` | Agent-only models (`targetChatSessionType: "copilotcli"`) | Agents window, Manage panel (hidden by default) |

### Settings (replaces `showInAgentsWindow`)

| Setting                                   | Default | Purpose                                                                           |
| ----------------------------------------- | ------- | --------------------------------------------------------------------------------- |
| `dekgo.agentsWindow`                 | `true`  | Register agent providers at runtime (controls whether Agents window works at all) |
| `dekgo.showAgentModelsInManagePanel` | `false` | Show agent vendors in the Manage Language Models panel                            |
...
When a user sets an API key for `dekgo` in the Manage panel:
...
```typescript
function providerVariant(base: DekProviderDefinition, agentVendor: AllProviderVendor): DekProviderDefinition;
```
...
3. **Manage panel clean by default** — agent vendors hidden via `when` clause (`"config.dekgo.showAgentModelsInManagePanel"`), not visible unless explicitly enabled
...
Users who set `dekgo.showInAgentsWindow: true` in v0.3.1:
...
| File                                                          | Change                                                                                                                                               |
| ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `package.json`                                                | Added `agentsWindow` and `showAgentModelsInManagePanel` configs; declared agent vendors (`dekgo-agent`, `dekzen-agent`) with `when` clause |
...
| `agentsWindow: false`                                       | General models only | ❌ No Dek models | General vendors only                        |
...
| VSIX build + install                                   | ✅ `1000bath.dek-copilot-chat@0.3.2`                                   |
| Chat view model picker                                 | ✅ Each model appears exactly once (no duplication)                           |
| Manage Language Models panel                           | ✅ Only `dekgo` + `dekzen` visible by default (no `-agent` vendors) |
...
4. **Default `true` for agent models is the right call** — user feedback from @hu3bi and @Wallacy confirmed that most users who discover the Agents window expect Dek models to be available without manual configuration.
...
_Resolved by @Marinski (PR [#42](https://github.com/1000bath/dek-copilot-chat/pull/42), hotfix) and @Wallacy (PR [#43](https://github.com/1000bath/dek-copilot-chat/pull/43), final solution). Issue [#41](https://github.com/1000bath/dek-copilot-chat/issues/41) reported by @hu3bi._
