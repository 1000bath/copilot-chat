...
**GitHub Issue:** [#49](https://github.com/1000bath/dek-copilot-chat/issues/49) — _[FEATURE] Add inline code suggestions with selectable AI model (Copilot-like experience)_  
**Related Branch (external):** [`Wallacy/dek-copilot-chat@feat/persistent-autocomplete`](https://github.com/Wallacy/dek-copilot-chat/tree/feat/persistent-autocomplete)  
...
[@Wallacy](https://github.com/Wallacy) already built a working prototype on a fork using VS Code's `InlineCompletionItemProvider` API, but concluded it is **not viable on the Dek Go provider** because every Go model is reasoning-first and produces 400–700 hidden reasoning tokens before any visible output, pushing latency to 5–10 seconds.

This document captures fresh external research (2026-06-17) on:

1. Whether the Dek Go blocker is fundamental or solvable today.
2. Which providers/models/endpoints can realistically deliver a sub-2-second autocomplete experience.
3. A concrete, prioritized execution plan that does **not** depend on waiting for Dek to add a `/completions` endpoint.

---

## Problem Recap — Why Dek Go Cannot Power Autocomplete

Findings from the prototype branch, confirmed against current provider behavior:

| Symptom                | Detail                                                                                                                                                                                                                 |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Latency floor ~5–10s   | All Dek Go models are reasoning-first. Even with `reasoning: false`, MiMo V2.5 and DeepSeek V4 Flash emit 400–700 hidden reasoning tokens before visible text.                                                    |
| TPS ceiling ~60        | At ~60 tokens/sec, even a small completion consumes 1–2s just for output, on top of reasoning time.                                                                                                                    |
| Fast models too costly | GLM, Kimi, Qwen via Dek Go are priced for agentic coding, not for高频 autocomplete calls.                                                                                                                         |
| No FIM endpoint        | Dek exposes only `/chat/completions`. No `/completions` (FIM) endpoint, no `prefix` / `suffix` / `echo` / `logprobs`. Forces a "fake chat" prompt that is slower and less accurate than a dedicated FIM endpoint. |

**Conclusion:** The blocker is **intrinsic to the Dek Go ecosystem today**. The fastest path to a shippable feature is to source autocomplete from providers that (a) ship a non-reasoning code model and (b) expose a native FIM endpoint — independent of Dek Go.
...
- **Non-thinking mode = no hidden reasoning tokens** → directly solves the Dek Go blocker.
...
| ⛔ Skip  | Dek Go models for autocomplete                       | —      | 5–10s            | Reasoning-first                 | Intrinsic blocker; revisit only if Dek ships `/completions` + non-reasoning model |
...
| Criterion              | DeepSeek FIM           | Qwen-NIM                    | Ollama local         | Dek Go                    |
| ---------------------- | ---------------------- | --------------------------- | -------------------- | ------------------------------ |
...
- Dek provider docs: <https://opencode.ai/docs/providers>
- Prototype branch (prior art): <https://github.com/Wallacy/dek-copilot-chat/tree/feat/persistent-autocomplete>
