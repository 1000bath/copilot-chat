...
# PR #1 Review, Test, and Merge — dekgo.freeOnly Setting
...
This document records the full session in which the first external community Pull Request to the Dek Copilot Chat extension was reviewed, tested locally, and merged. The session covered open-source maintainer workflow, code review, local testing, branch management, and documentation.
...
**Problem:** First external community PR (#1) was submitted by [@Wallacy](https://github.com/Wallacy) from fork `Wallacy/dek-copilot-chat`. As a first-time open-source maintainer, the project owner needed to understand the PR, evaluate its quality, and decide whether to merge.

**PR Details:**

| Item        | Value                                                     |
| ----------- | --------------------------------------------------------- |
| **Title**   | `feat: add dekgo.freeOnly to expose paid Zen models` |
...
2. **Quality Assessment:**
   ...
   | Naming Convention      | ✅ Excellent | `dekgo.freeOnly` consistent with existing settings |
...
4. **Important Clarification** — The `freeOnly` setting and the API key are **independent**:
   ...
   - API key controls **whether requests are accepted** by Dek servers
...
3. Merged `main` into `develop` with `--no-ff`:
   ...
   git merge --no-ff main -m "Merge main (PR #1: add dekgo.freeOnly) into develop"
   # → Commit 679c224
...
```text
*   679c224 (develop) Merge main (PR #1: add dekgo.freeOnly) into develop
|\
| *   bb10020 (origin/main, main) Merge pull request #1 from Wallacy/feat/zen-freeOnly-config
| |\
| | * 0565385 feat: add dekgo.freeOnly to expose paid Zen models
| |/
...
> Merged ✅
>
> The `dekgo.freeOnly` setting is a great addition: it keeps backward compatibility (`true` by default) while giving users the option to expose paid Zen models when needed. Also appreciate the clean documentation updates in README and CHANGELOG.
...
| File               | Change                                                      | Impact                                 |
| ------------------ | ----------------------------------------------------------- | -------------------------------------- |
| `src/extension.ts` | `filterModel` reads `dekgo.freeOnly` setting           | User-controllable Zen model visibility |
| `package.json`     | Added `dekgo.freeOnly` setting                         | Appears in VS Code Settings UI         |
...
_The `dekgo.freeOnly` feature was later included in v0.1.4 (2026-05-17)._
