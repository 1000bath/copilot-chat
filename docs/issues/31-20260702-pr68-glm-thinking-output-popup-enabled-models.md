...
1. **#61 — GLM thinking accepts invalid values.** The `dekgo.thinking.glm` setting only offered `on`/`off`, but GLM 5.2 (per models.dev) requires `reasoning_effort` with values `high`/`max`, not a `thinking.type` toggle. Selecting `"on"` sent `thinking: { type: "enabled" }` to the gateway, which GLM 5.2 silently ignores or rejects.
...
**Fixed in:** v0.3.5 (PR [#68](https://github.com/1000bath/dek-copilot-chat/pull/68), merged 2026-07-02)
**Issues:** [#61](https://github.com/1000bath/dek-copilot-chat/issues/61), [#62](https://github.com/1000bath/dek-copilot-chat/issues/62), [#67](https://github.com/1000bath/dek-copilot-chat/issues/67)
