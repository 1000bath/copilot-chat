...
1. **#59 — Subscription cost drift.** The usage tracker estimated costs locally from token counts × model pricing, but this drifted 9–15% from actual billing because it couldn't see server-side cache affinity or CLI/TUI usage. The Dek CLI writes actual billed amounts to `~/.local/share/opencode/opencode.db`, but the tracker never read it.
...
**Fixed in:** v0.3.5 (PR [#60](https://github.com/1000bath/dek-copilot-chat/pull/60), merged 2026-06-30)
**Issues:** [#59](https://github.com/1000bath/dek-copilot-chat/issues/59)
...
- **CLI/TUI usage** — requests made through the Dek CLI or TUI are billed but never recorded by the extension
- **Pricing drift** — model pricing changes on the server side aren't reflected until the extension updates its bundled snapshot

The result: status bar percentages drifted 9–15% from what users actually saw on their Dek billing page.
...
The SQLite reader (`readDekHistory()`) and row aggregator (`buildSummaryFromRows()`) already existed in `goUsageTracker.ts` but were dead code. `getSummary()` never called them. The extension only used `buildSummaryFromTracked()`, which relied on locally estimated costs.
...
1. Calls `readDekHistory()` which reads `~/.local/share/opencode/opencode.db` via `sqlite3` CLI
...
```typescript
// getSummary() now tries SQLite first
const sqliteRows = readDekHistory();
if (sqliteRows) {
...
- **Double DB read:** `hasSQLiteData` getter and `getSummary()` both call `readDekHistory()` independently. If both fire close together, two `sqlite3` processes spawn for the same read. Low priority; could cache per-call if profiling shows it matters.
