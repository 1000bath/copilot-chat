# Issue #106 — Dek Zen models listed twice with native BYOK group

**Date:** 2026-08-05
**Status:** ✅ Resolved
**Related:** GitHub Issue [#106](https://github.com/1000bath/dek-copilot-chat/issues/106)
...
With the API key configured via VS Code's native Manage Models / BYOK flow, every Dek Zen model appears twice: `vscode.lm.selectChatModels({ vendor: "dekzen" })` returns **16** models instead of **8**.

## Root Cause

VS Code resolves a provider that declares a `configuration` schema in two passes: once **without** a group (`configuration` undefined), then once **per configured group** (with the group's resolved `apiKey`). It namespaces model identifiers by group (`toModelIdentifier` → `dekzen/<id>` vs `dekzen/<group>/<id>`), so both sets land in `_modelCache` and are never deduplicated.
...
- GitHub Issue: [#106](https://github.com/1000bath/dek-copilot-chat/issues/106)
- Related fix (introduced the fallback that enables the duplication): [#86](https://github.com/1000bath/dek-copilot-chat/issues/86) — `docs/issues/43-20260803-issue86-zen-nonagent-0-models.md`
