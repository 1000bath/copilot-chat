...
Full review and community feedback for contributor PR #18 by [Wallacy](https://github.com/Wallacy), which fixes the Kimi (MoonshotAI) thinking payload format. The extension was sending `enable_thinking: true | false` but the Dek Go gateway rejects this field with HTTP 400: "Extra inputs are not permitted". The correct format is `thinking: { type: "enabled" | "disabled" }` — matching the GLM family format.

**PR:** [1000bath/dek-copilot-chat#18](https://github.com/1000bath/dek-copilot-chat/pull/18)  
...
The `dekgo.thinking.kimi` setting was documented and implemented to send `enable_thinking: true | false` in the request payload. However, the Dek Go gateway **rejects** this field with:
...
| #   | Change                     | Detail                                                                                                                                |
| --- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| P3  | Setting description update | `dekgo.thinking.kimi` description updated from `enable_thinking: true \| false` to `thinking: { type: 'enabled' \| 'disabled' }` |
...
```bash
# Fetch PR metadata
gh pr view 18 --repo 1000bath/dek-copilot-chat --json title,body,state,author,headRefName,baseRefName,additions,deletions,changedFiles,files,commits,reviewDecision,mergeable,statusCheckRollup,labels
...
Review comment posted to PR #18 on 2026-06-11: [comment link](https://github.com/1000bath/dek-copilot-chat/pull/18#issuecomment-4679210157)
...
1. **Gateway payload formats vary** — the Dek Go gateway validates request fields strictly. `enable_thinking` is rejected as an "extra input" for Kimi even though it's the MoonshotAI-native format. The gateway normalizes everything through its own `transform.ts`.
