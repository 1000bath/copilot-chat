...
**Shipped in:** v0.3.3 (PR [#50](https://github.com/1000bath/dek-copilot-chat/pull/50), merged 2026-06-17)
**Issues:** [#23](https://github.com/1000bath/dek-copilot-chat/issues/23)
...
Users who topped up mid-cycle, or whose real Dek Go dashboard showed a different spent amount than the client-side estimate, had no way to reconcile the tracker with reality. The displayed percentages drifted from the actual subscription state, making the usage indicator unreliable.
...
New command `dekgo.setUsageTargets` registered in `extension.ts`:
...
- Tooltip (`buildUsageTooltip`) adds command link: `[$(pencil) Set spent targets](command:dekgo.setUsageTargets)` with `md.isTrusted = true`.
