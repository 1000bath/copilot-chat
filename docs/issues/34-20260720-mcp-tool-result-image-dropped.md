...
**Issues:** [#77](https://github.com/1000bath/dek-copilot-chat/issues/77)
...
Reported in [#77](https://github.com/1000bath/dek-copilot-chat/issues/77)
...
- `provideLanguageModelChatInformation` previously logged one line per model
  ...
  log line: `Models registered: count=22 provider=dekgo first=… last=…`.
- `fetchModels` previously called `vscode.window.showWarningMessage` on every
  transient upstream failure (400/503 from Dek's shared gateway are
...
`mimo-v2.5` produced a series of `400 Bad Request` errors from the Dek Go
...
- A single transient 400 from the shared Dek gateway on the auto-registered
  `dekzen-agent` provider (the user was on `dekgo`) triggered a modal
...
3. Picked an Dek Go model in Copilot Chat.
