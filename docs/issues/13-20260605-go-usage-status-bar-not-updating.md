...
After installing v0.2.0 and using Dek Go models in Copilot Chat, the status bar item `Go: 0%·0%·0%` did not change. No usage data was being recorded despite confirmed model usage.
...
**Discovery:** Initial implementation used SQLite-first approach — reading `~/.local/share/opencode/opencode.db` to get server-computed cost data. This required the user to have run Dek CLI (TUI) at least once.
...
Searched all possible Dek endpoints for programmatic usage data:
...
**Result:** `getSummary()` now exclusively uses extension-tracked data. SQLite reader `readDekHistory()` kept as dead code for potential future enrichment.
...
**Output channel:** "Dek Go Usage" — visible in VS Code Output panel.
...
code --install-extension dek-copilot-chat-0.2.1.vsix --force
```
...
3. Check Output panel → "Dek Go Usage" channel
...
1. **No external dependency for billing** — Dek has no public billing API. Extension-tracked estimation is the only option.
